...
const CreateLoanForm = () => {
  ...
  const [showChooseGuarantorModal, setShowChooseGuarantorModal] =
    useState(true);
  ...
  const handleSubmit = async (e) => {
    ...
    if (useGuarantor) {
      ...
      if (amount > balance) {
        setServerResponse(
          "You can apply for up to 100% of your balance when using a guarantor."
        );
        return;
      }
    } else {
      ...
      if (amount > balance * 0.9) {
        ...
        setServerResponse(
          `You can apply for up to ${balance * 0.9} when not using a guarantor.`
        );
        return;
      }
    }
  ...
  const createLoan = async () => {
    try {
      ...
      const response = await axios.post(`${config.BASE_API_URL}/createLoan`, {
        userId: user.userId,
        amount,
        interestRate,
        dueDate,
      ...
      if (token) {
        ...
        try {
          ...
          const { data } = await axios.get(
            `${config.BASE_API_URL}/balance/${decoded.userId}`
          );
  ...
  const addGuarantorsToLoan = async (loanId) => {
    ...
    try {
      const response = await axios.post(
        `${config.BASE_API_URL}/addGuarantorsToLoan`,
        { userId: user.userId, loanId, guarantors }
      );
      setServerResponse(response.data.message);
      setIsModalOpen(false);
    } catch (error) {
      console.error(error);
      setServerResponse(
        "An error occurred while adding the guarantors to the loan."
      );
    }
  };
  ...
  const addGuarantor = async (e) => {
    ...
    try {
      ...
      const guarantorRes = await axios.get(
        `${config.BASE_API_URL}/user/${guarantorID}`
      );
      ...
      if (response.data.message === "Email sent successfully") {
        ...
        setVerifiedGuarantors([
          ...verifiedGuarantors,
          { id: guarantorID, name: guarantorName, isVerified: true },
        ]);
        setServerResponse(response.data.message);
      } else {
  ...
  const removeGuarantor = (id) => {
    ...
    const newVerifiedGuarantors = verifiedGuarantors.filter(
      ...
      <div className={"flex flex-wrap"}>
        {useGuarantor ? (
          <>
            <div
              className={
                "w-full flex flex-col justify-center items-center md:w-1/3 lg:w-1/3"
              }
            >
              ...
              {guarantors.length > 0 && (
                <ul className={"max-w-xs w-full"}>
                  {verifiedGuarantors.map((guarantor) => (
                    <li
                      className={
                        "flex gap-3 w-full mb-3 items-center justify-between"
                      }
                      key={guarantor.id}
                    >
                      <input
              ...
              {useGuarantor && guarantors.length < 3 ? (
                <form
                  className={
                    "flex w-full max-w-xs gap-3 items-center justify-center"
                  }
                ...
                >
                  ...
                  <button
                    className={
                      "btn btn-circle ring-offset-1 border-2 border-warning bg-warning text-white text-2xl ring-2 ring-inset ring-white hover:bg-red-700 hover:border-red-700"
                    }
                    type={"submit"}
                  >
        ...
        <div
          className={
            useGuarantor
              ? "w-full md:w-2/3 lg:w-2/3 flex flex-col justify-center items-center"
        ...
        >
          ...
          <form
            className="w-full justify-center h-full pt-7 items-center flex flex-col gap-2"
          ...
          {serverResponse && (
            <div className="mt-4 text-green-500">{serverResponse}</div>
          )}
          <Modal
            isOpen={isModalOpen}
          ...
          >
            <h2>Confirm Loan</h2>
            <p>
              You will pay a total amount of {totalAmount}. Do you want to
              proceed?
            </p>
          ...
          >
            ...
            <div className="flex flex-row gap-4">
              <button
                className="btn btn-circle ring-offset-1 border-2 border-warning bg-warning text-white text-2xl ring-2 ring-inset ring-white hover:bg-red-700 hover:border-red-70"
                onClick={() => {
                  setUseGuarantor(true);
                  setShowChooseGuarantorModal(false);
                }}
              >
                <i className="fas fa-check"></i>
              </button>
